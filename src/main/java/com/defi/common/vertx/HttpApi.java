...
/**
 * {@code HttpApi} provides utility methods for handling HTTP requests
 * asynchronously and synchronously
 * using Vert.x framework. It wraps the request handling into async and sync
 * handlers, which are used to
 * process requests and respond with appropriate results.
 *
 * <p>
 * This class provides two main methods for handling requests:
 * {@code handleAsync} and {@code handleSync}.
 * Both methods wrap a function that performs the actual business logic, but
 * {@code handleAsync} runs
 * the logic asynchronously, while {@code handleSync} runs it synchronously.
 * </p>
 */
public class HttpApi {
     ...
     * This method uses Vert.x's event loop to execute blocking functions
     * asynchronously.
     *
     * <p>
     * The result of the function is then passed to the HTTP response. In case of
     * failure,
     * the error is handled by {@code handleError} method.
     * </p>
     *
     * @param blockingFunction The function to execute asynchronously. It takes a
     *                         {@code RoutingContext}
     *                         and returns a response object.
     * @param <T>              The type of the response.
     * @return A Vert.x {@code Handler<RoutingContext>} that processes the request
     *         asynchronously.
     */
     ...
     *
     * <p>
     * The result of the function is sent as the response body. If an error occurs
     * during the execution,
     * it is caught and processed by {@code handleError} method.
     * </p>
     *
     * @param syncFunction The function to execute synchronously. It takes a
     *                     {@code RoutingContext}
     *                     and returns a response object.
     * @param <T>          The type of the response.
     * @return A Vert.x {@code Handler<RoutingContext>} that processes the request
    ...
    private static void handleError(RoutingContext ctx, Throwable err) {
        // Send the generic error response to the client
